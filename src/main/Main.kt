...
private fun day13() {
    ...
    val inputConverted = mutableListOf<Area>()
    input.forEach {
        val index = Integer.parseInt(it.split(":")[0])
        val value = Integer.parseInt(it.split(":")[1].trim())

        inputConverted.add(Area(index, value))
    }

    val time = measureTimeMillis {
        println("Result ${d.calcPart2(inputConverted)}")
    }
    println("time in millis: $time")
...
private fun day21() {
    val d = Day21()
    val input = parseInput("src/main/day21/input", ListOfStringParser())
...
private fun day23() {
    val d = Day23()
    val input = parseInput("src/main/day23/input", ListOfStringParser())

    var time = measureTimeMillis {
        println("Result part 1: ${d.calcPart1(input)}")
    }
    println("time in millis: $time")

    time = measureTimeMillis {
        println("Result part 2: ${d.calcPart2()}")
    }
    println("time in millis: $time")
}
...
private fun day24() {
    val d = Day24()
    val input = parseInput("src/main/day24/input", ListOfStringParser()).map { Pair(Integer.valueOf(it.split("/")[0]), Integer.valueOf(it.split("/")[1])) }.toMutableList()
